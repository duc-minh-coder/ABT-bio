...
export default function CartView({
  ...
  const handleApplyPromo = () => {
    ...
    if (code === "ABT10") {
      ...
      setPromoSuccess(
        "Áp dụng thành công mã giảm 10% đối với thiết bị y tế ABT!",
      );
    } else if (code === "BIOLAB") {
      onApplyPromoCode(0.15, "BIOLAB"); // 15% discount for academic laboratory
      setPromoSuccess(
        "Kích hoạt mã ưu đãi 15% dành cho Phòng nghiên cứu học thuật!",
      );
    } else if (code === "") {
      ...
      <div className="mx-auto max-w-2xl px-4 py-16 text-center space-y-6">
        ...
        <div className="space-y-2">
          <h2 className="text-xl font-bold text-slate-900 dark:text-white">
            Giỏ Hàng Chưa Có Thiết Bị
          </h2>
          <p className="text-sm text-slate-500 max-w-sm mx-auto leading-relaxed">
            Vui lòng tham khảo danh mục sản phẩm y sinh của chúng tôi và lựa
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-4 space-y-8">
      ...
      <div className="text-left space-y-2 border-b pb-4">
        ...
        <p className="text-xs text-slate-505 text-slate-500 font-mono">
          Bảng báo giá dự kiến & Kiểm tra thông số thanh toán
        </p>
      </div>
          ...
          <div className="overflow-hidden bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-850 rounded-2xl shadow-sm">
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-850">
              {cartItems.map((item) => (
                <div
                  key={item.product.id}
                  className="grid sm:grid-cols-12 gap-4 p-4 sm:items-center text-left"
                >
                  {/* Photo & Name */}
                  ...
                  <div className="col-span-12 sm:col-span-2 text-left sm:text-center mt-2 sm:mt-0">
                    <span className="text-xs text-slate-400 sm:hidden block font-mono">
                      Đơn giá:
                    </span>
          ...
          <div className="p-4 rounded-xl bg-cyan-950/10 border border-cyan-800/10 text-slate-600 dark:text-slate-350 text-xs flex items-start gap-3">
            ...
            <div className="space-y-1">
              <span className="font-bold text-slate-800 dark:text-slate-200 block">
                Cam kết chính sách ủy quyền từ ABT Corp:
              </span>
              <p className="leading-relaxed">
                100% linh kiện thay thế chuẩn CO/CQ gốc. Chuyên viên y sinh bảo
                dưỡng tận nơi nhanh chóng trong vòng 8-24 tiếng sau khi nhận
                thông báo sự cố kỹ thuật.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 space-y-6 text-left">
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-850 p-6 rounded-2xl shadow-sm space-y-6">
            ...
            <div className="space-y-3.5 text-xs text-slate-600 dark:text-slate-300">
              ...
              {shippingFee > 0 && (
                <p className="text-[10px] text-slate-400 leading-tight">
                  Mẹo: Đơn hàng thiết bị y sinh trên 100 Triệu đ sẽ được ABT
                  miễn phí vận chuyển & hỗ trợ lắp đặt phòng thí nghiệm.
                </p>
              )}
            ...
            <div className="pt-4 border-t space-y-2.5">
              ...
              {promoSuccess && (
                <p className="text-[10px] text-emerald-600 dark:text-emerald-400 leading-relaxed font-sans">
                  {promoSuccess}
                </p>
              )}
              {promoError && (
