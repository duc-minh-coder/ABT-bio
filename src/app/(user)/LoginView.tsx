...
export default function LoginView({
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    if (!email || !password) {
      setErrorLocal("Vui lòng cung cấp email và mật khẩu đăng nhập.");
      return;
    }
    ...
    try {
      const authResult = await loginUser(trimmedEmail, password);
      setStoredAuthTokens(authResult.token, authResult.refreshToken);
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      await registerUser(name, trimmedEmail, password);
      const authResult = await loginUser(trimmedEmail, password);
      setStoredAuthTokens(authResult.token, authResult.refreshToken);
      const backendUser = await getCurrentUser();

      onAddUser(backendUser);
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-150 dark:border-slate-850 shadow-2xl p-8 text-left relative overflow-hidden backdrop-blur-md">
        ...
        <div className="text-center space-y-3 mb-8">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">
              Đăng nhập phân quyền để truy cập tính năng tương ứng
            </p>
          </div>
        </div>
        ...
        {!isRegisterMode ? (
          <form onSubmit={handleSignIn} className="space-y-4">
            <div>
              ...
              <p className="text-[10px] text-slate-400 mt-1">
                Sử dụng tài khoản mẫu bên dưới để đăng nhập tức thì.
              </p>
            </div>
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 mt-1">
                Vai trò Admin cho phép chỉnh sửa danh sách sản phẩm, phê duyệt
                và quản lý toàn cơ sở dữ liệu.
              </p>
            </div>
        ...
        <div className="mt-6 p-4 bg-slate-50 dark:bg-slate-950 rounded-2xl border border-slate-100 dark:border-slate-850 text-xs">
          <span className="font-extrabold text-[10px] font-mono text-slate-400 uppercase tracking-widest block mb-2.5 flex items-center gap-1">
            <Sparkles className="h-3 w-3 text-amber-500" /> Click Đăng Nhập
            Nhanh Để Kiểm Thử Môn Học
          </span>
          <div className="space-y-2">
            ...
            >
              <div>
                ...
                <span className="text-[10px] font-mono opacity-80">
                  admin@abt-biomedical.vn
                </span>
              </div>
              <span className="text-[9px] font-extrabold uppercase px-2 py-0.5 rounded bg-rose-500 text-white">
                Chức năng Admin
              </span>
            </button>
            ...
            >
              <div>
                ...
                <span className="text-[10px] font-mono opacity-80">
                  user@abt-biomedical.vn
                </span>
              </div>
              <span className="text-[9px] font-extrabold uppercase px-2 py-0.5 rounded bg-cyan-500 text-white">
                Chức năng User
              </span>
            </button>
            ...
            >
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-mono font-bold text-slate-400">
                  Tài khoản Bị Khóa Demo
                </span>
              </div>
              <span className="text-[9px] opacity-75">
                blocked@abt-biomedical.vn
              </span>
            </button>
          </div>
        </div>
