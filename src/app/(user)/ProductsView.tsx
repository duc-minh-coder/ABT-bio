...
import { CATEGORIES, BIOMEDICAL_PRODUCTS } from '../../data';
import { Product } from '../../types';
import { Search, SlidersHorizontal, Check, CheckCircle2, ShieldAlert, Award, FileText, ShoppingCart, Truck, RefreshCw, X } from 'lucide-react';
...
export default function ProductsView({
  ...
  const filteredProducts = useMemo(() => {
    let result = [...BIOMEDICAL_PRODUCTS];
    ...
    if (selectedPricePreset === 'under_50m') {
      ...
      result = result.filter((p) => p.price >= 50000000 && p.price <= 150000000);
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-4 space-y-8">
      ...
      <div className="text-left space-y-2">
        <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white">Danh Mục Thiết Bị Y Sinh</h1>
        <p className="text-xs text-slate-500 font-mono tracking-widest uppercase">Trữ lượng sẵn có • Bảo hành 24 tháng chính hãng ABT • Chuyên nghiệp & Chuẩn xác</p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-lg space-y-4 text-slate-200">
        <div className="grid md:grid-cols-12 gap-4 items-center">
          ...
          <div className="md:col-span-4 flex flex-wrap gap-2">
            {pricePresets.map((preset) => (
              <button
                ...
                key={preset.value}
                onClick={() => setSelectedPricePreset(preset.value as PricePreset)}
          ...
          <div className="md:col-span-3 flex items-center gap-2">
            <span className="text-xs text-slate-405 text-slate-400 inline-block shrink-0 font-mono">Sắp xếp:</span>
            <select
              id="sort-select"
              value={sortBy}
        ...
        <div className="pt-4 border-t border-slate-800 flex flex-wrap gap-2 items-center">
          <span className="text-xs text-slate-400 font-mono inline-block mr-2 uppercase tracking-wide">Phân nhóm:</span>
          {CATEGORIES.map((cat) => (
            <button
              id={`cat-filter-${cat}`}
              key={cat}
      ...
      {filteredProducts.length === 0 ? (
        <div className="p-12 text-center bg-slate-50 dark:bg-slate-900 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-850 space-y-3">
          <p className="text-slate-600 dark:text-slate-400 font-medium">Không tìm thấy thiết bị y sinh nào khớp với bộ lọc hiện tại.</p>
          <button
          ...
          {filteredProducts.map((product) => (
            ...
            >
              <div className="relative aspect-video overflow-hidden border-b border-slate-100 dark:border-slate-850 bg-slate-950">
                ...
                {/* Stock Warning Badge */}
                <span className={`absolute bottom-3 left-3 text-[10px] font-bold px-2 py-0.5 rounded shadow-sm flex items-center gap-1 ${
                  product.stock <= 3 
                    ? 'bg-amber-100 dark:bg-amber-950 text-amber-700 dark:text-amber-300' 
                    : 'bg-emerald-100 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300'
                }`}>
                  <span className={`h-1.5 w-1.5 rounded-full ${product.stock <= 3 ? 'bg-amber-500 animate-pulse' : 'bg-emerald-500'}`} />
                  {product.stock <= 3 ? `Chỉ còn ${product.stock} ${product.unit}` : `Mẫu sẵn hàng (${product.stock} ${product.unit})`}
                </span>
              ...
              <div className="p-6 flex-1 flex flex-col justify-between text-left">
                <div className="space-y-3">
                  <div className="flex justify-between items-start gap-1">
                    <span className="text-slate-400 text-[11px] font-mono">{product.id}</span>
                    <span className="text-xs text-sky-600 font-medium">Bảo hành 2 năm</span>
                  </div>
                ...
                <div className="pt-5 mt-5 border-t border-slate-100 dark:border-slate-850 space-y-4">
                  <div className="flex justify-between items-baseline">
                    <span className="text-xs text-slate-500">Giá phân phối:</span>
                    <span className="text-lg font-extrabold text-teal-600 dark:text-teal-400">
                      {product.price.toLocaleString("vi-VN")} đ
                      <span className="text-xs font-normal text-slate-400 block sm:inline"> /{product.unit}</span>
                    </span>
                  </div>
            ...
            >
              ...
              <div className="space-y-5">
                <div className="pr-12">
                  ...
                  <p className="text-xs text-slate-400 font-mono mt-1">Hãng ABT Corp • Tiêu chuẩn Y tế Việt Nam</p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-12 gap-4 sm:gap-6">
                  ...
                  <div className="sm:col-span-7 space-y-3.5">
                    <div className="p-4 bg-cyan-50/55 dark:bg-cyan-950/20 border border-cyan-100/65 dark:border-cyan-900/40 rounded-2xl">
                      <span className="text-[10px] text-slate-400 uppercase block font-bold leading-none mb-1">Yêu cầu báo giá chính thức:</span>
                      ...
                      <span className="text-[10px] text-slate-400 font-mono block mt-1">/ {selectedProductDetail.unit} (lắp đặt tận phòng sạch Lab)</span>
                    </div>

                    <div className="grid grid-cols-2 gap-3 text-xs">
                      <div className="bg-slate-50 dark:bg-slate-950 p-2.5 rounded-xl border border-slate-100 dark:border-slate-850">
                        <span className="text-slate-400 block font-mono text-[9px] sm:text-[10px] uppercase">Trạng thái:</span>
                        <span className="font-extrabold text-teal-600 dark:text-teal-400">Còn {selectedProductDetail.stock} chiếc</span>
                      </div>
                      <div className="bg-slate-50 dark:bg-slate-950 p-2.5 rounded-xl border border-slate-100 dark:border-slate-850">
                        <span className="text-slate-400 block font-mono text-[9px] sm:text-[10px] uppercase">Vận chuyển:</span>
                        <span className="font-bold text-slate-700 dark:text-slate-300">Hoàn toàn miễn phí</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2.5">
                  <h4 className="text-[10px] sm:text-xs font-extrabold uppercase tracking-wider text-slate-400 dark:text-slate-500 border-b border-slate-100 dark:border-slate-800 pb-1.5 font-sans">Thông Số Kỹ Thuật Chi Tiết (Surgical Specs)</h4>
                ...
                <div className="p-3.5 bg-rose-50 dark:bg-rose-950/30 border border-rose-100 dark:border-rose-900/30 text-[11px] text-rose-700 dark:text-rose-300 rounded-xl flex items-start gap-2.5 leading-relaxed">
                  ...
                  <span>Sản phẩm này là trang thiết bị y học đặc thù, kỹ sư ABT sẽ bàn giao kèm Hồ sơ chất lượng CO/CQ, Phiếu kiểm định đo lường hiệu chuẩn quốc gia và Bộ hướng dẫn kỹ thuật SOP.</span>
                </div>
